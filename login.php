...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($username) && !empty($password)) {
        try {
            // BƯỚC 1: Chỉ tìm tài khoản dựa vào username
            $stmt = $pdo->prepare("SELECT * FROM users WHERE username = :username AND status = 'ACTIVE' LIMIT 1");
            $stmt->execute([
                ':username' => $username
            ]);
            ...
            // BƯỚC 2: Sử dụng password_verify để giải mã và kiểm tra mật khẩu
            if ($user && password_verify($password, $user['password_hash'])) {
                // THÊM DÒNG NÀY: Xóa nhãn dùng thử (nếu có) trước khi tạo phiên thật
                unset($_SESSION['role'], $_SESSION['username'], $_SESSION['toast_msg'], $_SESSION['toast_type']);
